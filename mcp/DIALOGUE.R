...
DIALOGUE1<-function(rA,k = 5,main,results.dir = "~/Desktop/DIALOGUE.results/",conf = "cellQ",
  ...
  X<-lapply(rA, function(r){ return(r@X) })
  ...
  for(x in cell.types){
    ...
    # Residuals after linear regression between latent variable (MCP within current cell type) and confounding variable (cell quality)
    r@scores<-t(get.residuals(t(scores0),conf.m))
    R$cca.scores[[x]]<-r@scores
    R$cca.gene.cor1[[x]]<-cor(t(r@tpm),r@scores)
    # set of all genes that are among the most positively or negatively correlated with at least one MCP.
    g1<-sort(unique(unlist(get.top.cor(R$cca.gene.cor1[[x]],q = n.genes,min.ci = 0.05))))
    # Partial correlation of two variables (TPM and latent var) given a third (cell quality)
    # (limited to set of genes that are most strongly/negatively correlated with at least one MCP)
    R$cca.gene.cor[[x]]<-pcor.mat(t(r@tpm[g1,]),r@scores,r@cellQ) # tpm = (time, genes), score = (time, MCPs)
    C1<-R$cca.gene.cor[[x]]$R
    P1<-R$cca.gene.cor[[x]]$P
    C1[P1>(0.05/nrow(r@tpm))]<-0  # mark all genes without significant correlation as uncorrelated
...
DIALOGUE2.pair<-function(R,r1,r2,cell.types,results.dir){
  ...
  sig1<-R$cca.sig[[x1]]
  sig2<-R$cca.sig[[x2]]
  idx<-r1@samples # formerly intersection of abundant samples for each cell type.
  # subset data to shared samples (all time points, in our case)
  ...
  # Matches scores from r1 to corresponding samples in r2 (r1@tme.OE), and vice versa.
  ...
  f1<-function(sig1,sig2,x){
    p1<-DIALOGUE2.cell.mcp.corr(r2a,x,sig1)
    p2<-DIALOGUE2.cell.mcp.corr(r1a,x,sig2)
    #p1<-DIALOGUE2.mixed.effects(r2a,x,sig1,R$frm)
    #p2<-DIALOGUE2.mixed.effects(r1a,x,sig2,R$frm)
    
    sig1f<-intersect.list1(get.top.cor(p1[!is.na(p1$statistic),],q = 100,min.ci = 0.05),r1@genes)
    sig2f<-intersect.list1(get.top.cor(p2[!is.na(p2$statistic),],q = 100,min.ci = 0.05),r2@genes)
...
# Replacement for mixed effects model -- performs partial Spearman correlation between TMP of genes in cell type 2 and MCP in cell type 1.
DIALOGUE2.cell.mcp.corr<-function(r1,x,sig2){
  # r1 was a cell.type S4 that was converted to a list.
  genes<-unlist(sig2[paste0(x,c(".up",".down"))])
  b<-is.element(genes,rownames(r1$tme))
  X = r1$tme[genes[b],]
  Y = r1$scores[,x]
  P = get.mat(genes,c("statistic","p.value"))
  for(g in genes){
    p=cor.test(X[g,],Y,method="spearman",use="pairwise.complete.obs",alternative="two.sided")
    P[g,1]=p$statistic
    P[g,2]=p$p.value
  }
  P<-as.data.frame(P)
...
DLG.get.OE<-function(r1,r2,sig1,sig2,plot.flag = F,compute.scores = T){
  ...
  r1@scoresAv<-r1@scores
  r2@scoresAv<-r2@scores
