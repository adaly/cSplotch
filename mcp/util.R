...
# Given a (genes, MCPs) correlation matrix m, returns top q most strongly
# and negatively correlated genes for each MCP.
...
# Given a (genes, MCPs) correlation matrix m:
# - sort columns by increasing value
# - keep q smallest elements (paradoxically...)
# - return as a list with entries named by columns of correlation matrix
get.top.elements<-function (m,q = 100,min.ci = NULL,main = ""){
  ...
  for (i in 1:ncol(m)){
    ...
    # find value of q-th element after sorting (or last element, if q too large)
    ci <- mi[idx[min(q,length(mi))]]
    ci <- min(ci,min.ci)
    # keep only elements of column with value less than that of q-th
    b <- m[,i]<=ci
    b[is.na(m[,i])]<-F
